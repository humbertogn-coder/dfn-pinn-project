...
def assess(model):
    ...
    with torch.no_grad():
        prediction = model(p).reshape(rr.shape).numpy()
    ...
    with torch.no_grad():
        values = model(make_points(r2.ravel(), t2.ravel())).reshape(r2.shape)
...
def main():
    ...
    def closure():
        ...
        for p in (interior, initial, surface):
            p.grad = None
        losses = terms(model, interior, initial, surface)
    ...
    output = Path(__file__).resolve().parents[1]/"results"/("constant_flux_pinn_"+
