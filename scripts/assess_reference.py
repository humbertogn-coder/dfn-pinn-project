"""Apply provisional sampled criteria to x80 radial 160/320 DFN references."""
...
def main():
    ...
    for window, period, duration in (("startup", "0.05 seconds", 5), ("full", "10 seconds", None)):
        for radial in (160, 320):
            print(f"Assessing {window}: x80_r{radial}...", flush=True)
            solution, parameters, protocol, mesh, elapsed = run_discharge(
                80, period, radial_points=radial, duration_s=duration, rtol=1e-8, atol=1e-10)
            ...
            residuals = []
            inventory = signal("Total lithium in electrolyte [mol]")
            ...
            run_reports.append({"window": window, "radial_points": radial, "mesh": mesh,
            ...
            runs[(window, radial)] = (t.copy(), fields)
            del solution, variable, values, signal
            gc.collect()
    rows, curves = [], {}
    for window in ("startup", "full"):
        t0, a = runs[(window, 160)]
        t1, b = runs[(window, 320)]
    ...
    output = Path(__file__).resolve().parents[1] / "results" / (
        "reference_assessment_" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S%fZ"))
    output.mkdir(exist_ok=False)
    report = {"criteria_version": 1, "pybamm_version": pybamm.__version__,
              ...
              "overall_status": "PROVISIONAL: through-cell refinement and sampling audit pending",
              "scope": "Radial sampled checks only. Startup uses independent 5-second runs. "
                       "Later samples start at 10 seconds; 5-10 seconds and intersample extrema "
                       "are unresolved. Linear radial interpolation contributes error. Cutoff "
    ...
    for run in run_reports:
        print(f"{run['window']} r{run['radial_points']}: current_pass={run['current_pass']}, "
