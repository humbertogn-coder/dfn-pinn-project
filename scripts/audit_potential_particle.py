...
from dfn_pinn.potential_audit import audit, assess, reference_identity
...
def main():
    ...
    model = PotentialParticle(checkpoint["config"])
    ...
    hashes = reference_identity(reference_dir, c)
    if hashes != original["reference_hashes"]:
        raise ValueError("Reference files changed since training")
    metrics, sampling = audit(model, reference_dir)
    ...
    result = {**assessment, "metrics": metrics, "sampling": sampling, "limits": c["acceptance"],
              "reference_hashes": hashes, "run": str(run), "training_losses_reproduced": True,
              ...
              "auditor_hashes": {str(p.relative_to(root)): hashlib.sha256(p.read_bytes()).hexdigest()
                                  for p in (Path(__file__), root/"src/dfn_pinn/potential_audit.py")}}
