"""Train the specified manufactured particle; scientific audit is separate."""
...
import torch
from dfn_pinn.coupled_training import CoupledParticle, sample_points
...
def main():
    ...
    config_path = root/"configs/coupled_particle_v1.json"
    c = json.loads(config_path.read_text())
    c["loss_weights"]["kinetics"] = 10.0
    torch.manual_seed(c["seed"])
    torch.set_num_threads(1)
    model = CoupledParticle(c)
    points = sample_points(c, args.smoke)
    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S%fZ")
    output = (args.output_root or root/"results")/f"coupled_particle_kinetics10_{'smoke' if args.smoke else 'full'}_{stamp}"
    ...
    def loss():
        ...
        return total
    print(f"Manufactured coupled particle: {'SMOKE ONLY' if args.smoke else 'FULL BUDGET'}, float64 CPU", flush=True)
    ...
    replay = CoupledParticle(loaded["config"])
    ...
    report = {"status": "smoke_complete_not_scientific_acceptance" if args.smoke else "trained_pending_independent_audit",
              "config": c, "actual_adam_steps": steps, "lbfgs_max_iter": iterations,
